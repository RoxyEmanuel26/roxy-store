...
function getPercentageColor(percent: number) {
  if (percent >= 90) return 'bg-red-500'
  if (percent >= 70) return 'bg-amber-500'
  return 'bg-emerald-500'
}

function getPercentageTextColor(percent: number) {
  if (percent >= 90) return 'text-red-600 dark:text-red-400'
  if (percent >= 70) return 'text-amber-600 dark:text-amber-400'
  return 'text-emerald-600 dark:text-emerald-400'
}

function getPercentageBgColor(percent: number) {
  if (percent >= 90) return 'bg-red-50 dark:bg-red-950/20 text-red-700 dark:text-red-400 border-red-200'
  if (percent >= 70) return 'bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-400 border-amber-200'
  return 'bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400 border-emerald-200'
}
...
export default function QuotaUsagePage() {
  ...
  const dbPercent = Math.min(Math.round((dbUsedBytes / dbLimitBytes) * 100), 100)

  // Cloudinary Credit usage details
  const cloudCredits = data?.cloudinary?.credits
  const cloudPercent = cloudCredits ? Math.round(cloudCredits.used_percent) : 0
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-dark-surface rounded-2xl border border-brand-border/50 dark:border-dark-border p-5 shadow-sm flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              ...
              <p className="text-2xl font-bold text-brand-text dark:text-dark-text">
                {cloudCredits ? `${cloudCredits.usage} / ${cloudCredits.limit}` : 'N/A'}
              </p>
            </div>
        ...
        <div className="bg-white dark:bg-dark-surface rounded-2xl border border-brand-border/50 dark:border-dark-border p-5 shadow-sm flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              ...
              <p className="text-2xl font-bold text-brand-text dark:text-dark-text">
                {data?.redis.configured ? `${data.redis.keysCount} Keys` : 'Tidak Aktif'}
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-dark-surface rounded-2xl border border-brand-border/50 dark:border-dark-border shadow-sm overflow-hidden">
          <div className="p-5 border-b border-brand-border/50 dark:border-dark-border flex justify-between items-center">
            <div>
                  ...
                  {data.database.tables.map((table) => (
                    <TableRow key={table.name} className="hover:bg-gray-50/40 dark:hover:bg-dark-bg/10">
                      ...
                      <TableCell className="text-right font-medium">
                        {table.rowCount.toLocaleString('id-ID')} baris
                      </TableCell>
        ...
        <div className="bg-white dark:bg-dark-surface rounded-2xl border border-brand-border/50 dark:border-dark-border shadow-sm p-5 space-y-6">
          <div>
            <h2 className="font-bold text-base text-brand-text dark:text-dark-text flex items-center gap-1.5">
          ...
          <div className="space-y-4">
            ...
            {data?.cloudinary?.transformations && (
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs">
                  <span className="font-medium text-brand-text dark:text-dark-text">Transformasi Gambar</span>
                  <span className="text-brand-muted dark:text-dark-muted">
                    {data.cloudinary.transformations.usage.toLocaleString('id-ID')} / {data.cloudinary.transformations.limit.toLocaleString('id-ID')} ({data.cloudinary.transformations.used_percent}%)
                  </span>
                </div>
                <div className="w-full h-1.5 bg-gray-100 dark:bg-dark-border rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${getPercentageColor(data.cloudinary.transformations.used_percent)}`}
                    style={{ width: `${data.cloudinary.transformations.used_percent}%` }}
                  />
                </div>
              </div>
            )}
            ...
            {data?.cloudinary?.storage && (
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs">
                  <span className="font-medium text-brand-text dark:text-dark-text">Ruang Penyimpanan (Storage)</span>
                  <span className="text-brand-muted dark:text-dark-muted">
                    {formatBytes(data.cloudinary.storage.usage)} / {formatBytes(data.cloudinary.storage.limit)} ({data.cloudinary.storage.used_percent}%)
                  </span>
                </div>
                <div className="w-full h-1.5 bg-gray-100 dark:bg-dark-border rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${getPercentageColor(data.cloudinary.storage.used_percent)}`}
                    style={{ width: `${data.cloudinary.storage.used_percent}%` }}
                  />
                </div>
              </div>
            )}
            ...
            {data?.cloudinary?.bandwidth && (
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs">
                  <span className="font-medium text-brand-text dark:text-dark-text">Bandwidth Gambar</span>
                  <span className="text-brand-muted dark:text-dark-muted">
                    {formatBytes(data.cloudinary.bandwidth.usage)} / {formatBytes(data.cloudinary.bandwidth.limit)} ({data.cloudinary.bandwidth.used_percent}%)
                  </span>
                </div>
                <div className="w-full h-1.5 bg-gray-100 dark:bg-dark-border rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${getPercentageColor(data.cloudinary.bandwidth.used_percent)}`}
                    style={{ width: `${data.cloudinary.bandwidth.used_percent}%` }}
                  />
                </div>
              </div>
            )}
            ...
            {/* Resources count */}
            {data?.cloudinary?.resources !== undefined && (
              <div className="flex justify-between items-center text-xs pt-2 border-t border-brand-border/50 dark:border-dark-border">
                ...
                <span className="font-bold text-brand-text dark:text-dark-text">
                  {data.cloudinary.resources.toLocaleString('id-ID')} file
                </span>
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl border border-brand-border/50 dark:border-dark-border bg-gray-50/50 dark:bg-dark-bg/20 p-3 text-xs leading-relaxed text-brand-muted dark:text-dark-muted">
            💡 **Tips Hemat Cloudinary:** Pastikan tidak mengunggah ulang produk yang sudah ada agar hemat kapasitas penyimpanan (*Storage*) dan *Transformations* kredit Cloudinary Anda.
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-dark-surface rounded-2xl border border-brand-border/50 dark:border-dark-border shadow-sm p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-2">
          ...
          <div className="bg-gray-50/40 dark:bg-dark-bg/10 rounded-xl p-4 border border-brand-border/20 dark:border-dark-border/40">
            ...
            <p className="text-xs text-brand-muted dark:text-dark-muted mt-2">
              Jumlah request HTTP yang diproses oleh CDN Vercel.
            </p>
          </div>
          ...
          <div className="bg-gray-50/40 dark:bg-dark-bg/10 rounded-xl p-4 border border-brand-border/20 dark:border-dark-border/40">
            ...
            <p className="text-xs text-brand-muted dark:text-dark-muted mt-2">
              Waktu aktif pemrosesan serverless function API & dynamic routes.
            </p>
          </div>
          ...
          <div className="bg-gray-50/40 dark:bg-dark-bg/10 rounded-xl p-4 border border-brand-border/20 dark:border-dark-border/40">
            ...
            <p className="text-xs text-brand-muted dark:text-dark-muted mt-2">
              Bandwidth transfer data dari database/API ke Edge Network Vercel.
            </p>
          </div>
        </div>
        ...
        {!data?.vercel.liveStatsConfigured && (
          <div className="mt-4 p-4 rounded-xl border border-brand-border/50 bg-brand-surface/20 text-xs leading-relaxed text-brand-muted dark:text-dark-muted space-y-2">
            <p className="font-semibold text-brand-text dark:text-dark-text">🔗 Ingin Menghubungkan Statistik Vercel Secara Real-Time?</p>
            <p>
              Anda bisa melihat metrik penggunaan Vercel Anda secara langsung di halaman ini dengan menambahkan variabel lingkungan (*Environment Variables*) berikut ke dalam berkas **`.env.local`** (dan di dashboard Vercel settings):
            </p>
            <ul className="list-disc list-inside pl-2 space-y-1">
              <li>`VERCEL_API_TOKEN` - Token akses Vercel Anda (buat di Vercel Account Settings &gt; Tokens)</li>
              <li>`VERCEL_PROJECT_ID` - ID Proyek Vercel Anda (ditemukan di project settings)</li>
            </ul>
          </div>
        )}
      </div>
